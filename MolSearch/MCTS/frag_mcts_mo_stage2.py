...
parser.add_argument('--group_idx',
                    help='select a group of atoms to modify, for details search (start molecular settings) in the code,'
                         ' you will find the code for molecular selection',
                    type=int, default=0)
...
class State():
    ...
    def get_valid_actions(self):
        actions = get_actions(state=self.smiles, allow_substitution=True, allow_atom_addition=False,
                              allow_bond_addition=False)
        mo_actions = get_mo_stage2_actions(self.smiles, actions, functions=functions, thresholds=np.ones(n_obj) * 0.5,
                                           t=1.0)  # change 1 threshold to change
...
### start molecular settings
group_length = args.group_length  # change for different jobs
n_mols = len(start_mol_list)
start_idx = args.group_idx * group_length
end_idx = min((args.group_idx + 1) * group_length - 1, n_mols)

for mol_idx in range(start_idx, end_idx):
    model_str = args.goal + '_maxchild_' + str(args.max_child) + '_sim_' + str(args.num_sims) + '_scalar_' + str(
        args.scalar) + '_idx_' + str(mol_idx) + '_seed_' + str(args.seed)
