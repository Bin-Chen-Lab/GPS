    ...
    def __init__(self,
        ...
        if not empty:
            ...
            if self.dataset == 'bbbp':
                ...
                raw_file = pd.read_csv(self.root + '/raw/' + self.dataset +'.csv', sep=',')
                self.original_smiles = raw_file['smiles'].tolist()
            elif self.dataset == 'moonshot':
                raw_file = pd.read_csv(self.root + '/raw/3CL_Moonshot_activity_data_prep.csv', sep=',')
                self.original_smiles = raw_file['SMILES'].tolist()
            elif self.dataset == 'ncats':
                raw_file = pd.read_csv(self.root + '/raw/3CL_NCATS_enzymatic_activity_prep.csv', sep=',')
                self.original_smiles = raw_file['SMILES'].tolist()
            elif self.dataset == 'mooncats':
                raw_file = pd.read_csv(self.root + '/raw/mooncats_smiles.csv', sep=',')
                self.original_smiles = raw_file['SMILES'].tolist()
            elif self.dataset == 'dipg':
                raw_file = pd.read_csv(self.root + '/raw/screen_ZINC_metaDIPG_top2k.csv', sep=',')
                self.original_smiles = raw_file['SMILES'].tolist()
            elif self.dataset == 'swiss':
                pass ## HanMeng
            else:
...
#            print(key, item)
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            smiles_list, rdkit_mol_objs, labels = \
                _load_dipg_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
            ...
            ### modify based on dipg code above HanMeng
            pass
        ...
        # elif self.dataset == ''
            ...
            smiles_list, rdkit_mol_objs, labels = \
                _load_moonshot_dataset(os.path.join(self.root, 'raw', '3CL_Moonshot_activity_data_prep.csv'))
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
            ...
            smiles_list, rdkit_mol_objs, labels = \
                _load_ncats_dataset(os.path.join(self.root, 'raw', '3CL_NCATS_enzymatic_activity_prep.csv'))
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
            ...
            smiles_list_ncats, rdkit_mol_objs_ncats, labels_ncats = \
                _load_ncats_dataset(os.path.join("dataset/ncats", 'raw', '3CL_NCATS_enzymatic_activity_prep.csv'))
            smiles_list_moon, rdkit_mol_objs_moon, labels_moon = \
                _load_moonshot_dataset(os.path.join("dataset/moonshot", 'raw', '3CL_Moonshot_activity_data_prep.csv'))
            smiles_list = smiles_list_moon + smiles_list_ncats
            rdkit_mol_objs = rdkit_mol_objs_moon + rdkit_mol_objs_ncats
            labels = list(labels_moon) + list(labels_ncats)
            print(len(smiles_list), len(rdkit_mol_objs), len(labels))
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
                    ...
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
            df = pd.DataFrame(data_smiles_list, columns=["SMILES"])
            df.to_csv(self.root + '/raw/mooncats_smiles.csv', index=False)
        else:
...
def _load_dipg_dataset(input_path):
    """
...
def _load_swiss_dataset(input_path):
    #HanMeng
    """
    ...
    print(input_path)
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['SMILES']    # column name different, modify it 
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    ...
    preprocessed_smiles_list = [AllChem.MolToSmiles(m) if m != None else
                                None for m in preprocessed_rdkit_mol_objs_list]
    labels = np.zeros(len())    # assign 0 labels, will not used in this project
    # there are no nans
    assert len(smiles_list) == len(preprocessed_rdkit_mol_objs_list)
    assert len(smiles_list) == len(preprocessed_smiles_list)
    assert len(smiles_list) == len(labels)
...
def _load_moonshot_dataset(input_path):
    """
    ...
    preprocessed_smiles_list = [AllChem.MolToSmiles(m) if m != None else
                                None for m in preprocessed_rdkit_mol_objs_list]
    labels = input_df['Active']
    # convert true to 1, false to -1, Nan to 0
    labels = labels.replace(True, 1)
    labels = labels.replace(False, -1)
    labels = labels.fillna(0)
    labels = labels.astype(int)
    v, counts = np.unique(labels.values, return_counts=True)
    print(v, counts)
    # there are no nans
    assert len(smiles_list) == len(preprocessed_rdkit_mol_objs_list)
    assert len(smiles_list) == len(preprocessed_smiles_list)
...
def _load_ncats_dataset(input_path):
    """
...
if __name__ == "__main__":
    ...
    # _load_moonshot_dataset('dataset/moonshot/raw/3CL_Moonshot_activity_data_prep.csv')
    # _load_ncats_dataset('dataset/ncats/raw/3CL_NCATS_enzymatic_activity_prep.csv')
    # dataset = MoleculeDataset("dataset/mooncats", dataset='mooncats', empty=True)
    # dataset = MoleculeDataset("dataset/dipg", dataset='dipg', empty=True)
    dataset = MoleculeDataset("dataset/swiss", dataset='swiss', empty=True)
