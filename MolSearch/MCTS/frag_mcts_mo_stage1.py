...
parser.add_argument('--max_child', help='maximum number of child nodes allowed', type=int, default=5)
parser.add_argument('--num_sims', help='number of iteration in simulation', type=int, default=20)
...
class State():
    ...
    def get_valid_actions(self):
        ...
        mo_actions = get_constraint_actions(actions, functions=functions, thresholds=np.array(self.score), t=args.t, l=self.level, L=MAX_LEVEL)
        # end of change
...
model_str = args.goal + '_maxchild_' + str(args.max_child) + '_sim_' + str(args.num_sims) + '_scalar_' + str(
    args.scalar) + '_t_' + str(args.t) +'_idx_' + str(args.mol_idx) + '_seed_' + str(args.seed)
